...
public class VariacaoRepository {
    ...
    public int updateEstoque(Integer id, Integer estoque) {
        return jdbcTemplate.update("UPDATE Variacao SET estoque = ? WHERE id = ?", estoque, id);
    }
    ...
    public Integer nextId() {
        Integer maxId = jdbcTemplate.queryForObject("SELECT COALESCE(MAX(id), 0) FROM Variacao", Integer.class);
